L.Control.SliderControl = L.Control.extend({
    ...
    startSlider: function () {
            ...
            slide: function (e, ui) {
                ...
                if(!!_options.markers[ui.value]) {
                    ...
                    if(_options.markers[ui.value].feature !== undefined) {
                        if(_options.markers[ui.value].feature.properties[_options.attribute]){
                            if(_options.markers[ui.value]) $('#slider-timestamp').html(
                                _extractTimestamp(_options.markers[ui.value].feature.properties[_options.attribute], _options));
                        }else {
                            console.error("Time property "+ _options.attribute +" not found in data");
                        }
                    }else {
                        // set by leaflet Vector Layers
                        if(_options.markers [ui.value].options[_options.attribute]){
                            if(_options.markers[ui.value]) $('#slider-timestamp').html(
                                _extractTimestamp(_options.markers[ui.value].options[_options.attribute], _options));
                        }else {
                            console.error("Time property "+ _options.attribute +" not found in data");
                        }
                    }
        ...
        if (!_options.range && _options.alwaysShowDate) {
            $('#slider-timestamp').html(_extractTimeStamp(_options.markers[index_start].feature.properties[_options.attribute], _options));
        }
